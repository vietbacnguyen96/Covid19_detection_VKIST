...
App.init = function () {
  ...
  function showInfo(images) {
    ...
    let template = `${Object.keys(images).map(file => `
      <div class="file file--${file}">
        <div class="name"><span>${images[file].name}</span></div>
      ...
      <img class="previewImg" src="${URL.createObjectURL(images[file])}">
    `).join("")}`;
    ...
    Object.keys(images).forEach(file => {
